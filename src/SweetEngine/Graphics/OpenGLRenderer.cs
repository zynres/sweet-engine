...
{
    public ShaderLoader _ShaderLoader;

    public CameraMixer* _CameraMixer;
    ...
    public OpenGLRenderer(Device* device, in Texture2DLoader textureLoader)
    {
        ...
        var shader = new Resources.Shaders.Shader();

        _ShaderLoader = new ShaderLoader(gl, shader.vertexSrc, shader.fragmentSrc);

        _CameraMixer =(CameraMixer*)NativeMemory.Alloc((nuint)sizeof(CameraMixer)); 
        *_CameraMixer = new CameraMixer(&device->Mouse, &device->Time);

        Desserts = new UnsafeList<Dessert>(100);
    ...
    public void AddObject(string path, Material* mat)
    {
        ...
        int modelLocation = gl.GetUniformLocation(_ShaderLoader.Id, "uModel");
        int mvpLocation = gl.GetUniformLocation(_ShaderLoader.Id, "uMVP");
    ...
    public void InitializeObjects()
    {
        ...
        _ShaderLoader.SetInt("uBaseMap", 0);
        _ShaderLoader.SetInt("uNormalMap", 1);
        _ShaderLoader.SetInt("uMetallicMap", 2);

        _ShaderLoader.SetVector3("uLightDir", new Vector3(0f, 1f, 1f));
        _ShaderLoader.SetFloat("uLightIntensity", 2f);

        frameBuffer = (FrameBuffer*)NativeMemory.Alloc((nuint)sizeof(FrameBuffer)); 
        *frameBuffer = new FrameBuffer(640, 320);

        SceneWindow.Depends(frameBuffer, _CameraMixer);

        for (uint i = 0; i < Desserts.Length; i++)
        {
            ref Dessert _object = ref Desserts[i];
    ...
    public void Render(in Intent intent)
    {
        ...
        _CameraMixer->Update(in intent);

        Matrix4x4 view = Matrix4x4.CreateLookAt(_CameraMixer->Transform.Position, _CameraMixer->Transform.Position + _CameraMixer->Transform.GetForward(), Vector3.UnitY);
        Matrix4x4 proj = Matrix4x4.CreatePerspectiveFieldOfView((float)Math.PI / 4f, _CameraMixer->Aspect, 0.1f, 1000f);

        _ShaderLoader.Use();

        _ShaderLoader.SetVector3("uViewPos", _CameraMixer->Transform.Position);

        for (uint i = 0; i < Desserts.Length; i++)
        {
            ref Dessert _object = ref Desserts[i];
            
            _ShaderLoader.SetVector4("uColor", _object.Renderer.material->Color);
            ...
            _ShaderLoader.SetMatrix4("uMVP", mvp);
            _ShaderLoader.SetMatrix4("uModel", model);
    ...
    public void Dispose()
    {
        ...
        NativeMemory.Free(_CameraMixer);
